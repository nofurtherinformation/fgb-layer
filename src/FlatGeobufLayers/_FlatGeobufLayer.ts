...
//   updateState({ changeFlags, props, oldProps, context }: UpdateParameters<this>) {
...
//       this.updateState({props, oldProps, context, changeFlags});
//       // this._setWGS84PropertyForTiles();
//     }
...
//     // this.setState({
//     //   renderedBounds: JSON.stringify({viewport: this.context.viewport})
//     // })
//     // @ts-ignore
...
// @ts-ignore
export default class FlatGeobufLayer extends CompositeLayer {
  ...
  // initializeState(props: any): void {
  //   console.log(props)
  // }
    // @ts-ignore
  ...
  private _updateFgbData(): void {
    // const { loadedViewport } = this.state;
    const { data } = this.props; //zRange, getFgbData 
  ...
  renderLayers() {
    const {data, renderSubLayers} = this.state;
    console.log(this)
